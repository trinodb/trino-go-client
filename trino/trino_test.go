...
func TestConfigFormatDSNTags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: "http://foobar@localhost:8080?session_properties=query_priority%3A1&source=trino-go-client&tags=test1%3Btest2%3Btest3",
		},
		{
			name: "single tag",
			config: &Config{
			...
			want: "http://foobar@localhost:8080?session_properties=query_priority%3A1&source=trino-go-client&tags=test1",
		},
		{
			...
			want: "http://foobar@localhost:8080?session_properties=query_priority%3A1&source=trino-go-client&tags=foo+%2520%3Bbar%3Dtest%3Bbaz%23tag",
		},
	}
